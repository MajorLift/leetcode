...
// Accepted 2022-12-22 02:35 UTC · JavaScript · 1658 ms · 51.4 MB
...
var exist = function(board, word) {
    ...
    const charCoordsMap = board.reduce((acc, row, i) => {
        row.forEach((char, j) => {
            if (wordCharsSet.has(char)) acc[char] = (acc[char] || new Set()).add(String([i, j]))
        })
        return acc
    }, {})
    ...
    function backtrack([x, y], idx) {
        ...
        charCoordsMap[word[idx]].delete(String([x, y]))
        for (const [i, j] of [[x - 1, y], [x + 1, y], [x, y - 1], [x, y + 1]]) {
            if (!charCoordsMap[word[idx + 1]].has(String([i, j]))) continue
            if (backtrack([i, j], idx + 1)) return true
        }
        charCoordsMap[word[idx]].add(String([x, y]))
        return false
    }
